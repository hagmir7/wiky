            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 p-8">
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="mt-8">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($book->tags as $tag)
                                <span
                                    class="bg-blue-100 text-blue-800 text-sm font-medium px-3 py-1.5 rounded-full">{{$tag->name}}</span>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="mt-10 flex flex-col sm:flex-row gap-4">
                        <button
                            class="flex-1 bg-blue-600 text-white px-8 py-3 rounded-lg font-semibold lg:hover:bg-blue-700 transition-colors duration-300">
                            Read Now
                        </button>


                        @php
                            $hasMedia = $book->hasMedia('books-file');
                        @endphp

                        <a @if($hasMedia) href="{{ $book->getFirstMediaUrl('books-file') }}" target="_blank" @endif
                        class="w-full text-center flex-1 px-8 py-3 rounded-lg font-semibold transition-colors duration-300 {{ $hasMedia ? 'bg-gray-100 text-gray-900 lg:hover:bg-gray-200' : 'bg-gray-300 text-gray-500 pointer-events-none opacity-50' }}">
                            Download PDF
                        </a>
